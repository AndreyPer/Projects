...
void testBigInteger() {
  ...
  if (oss.str() != "0 1234567") {
    throw std::runtime_error("Assignment from int, or stream output failed." + oss.str());
  }
  ...
  if (testString != "0 -1234567") {
    throw std::runtime_error("Unary minus or toString method failed. " + testString);
  }
  ...
  oss2 << a+5 << ' ' << 1-b; // 5 -1000 a+5 1-b
  if (oss2.str() != "5 -1000") {
    throw std::runtime_error("Arithmetic operations failed. " + oss2.str());
  }
  ...
  if (oss6.str() != "-10000001000000000000000000000000000000000") {
    throw std::runtime_error("Stream input or output failed." + oss6.str() + "d");
  }
  ...
  {
    throw std::runtime_error("Operator + failed. " + oss7.str());
  }
  ...
  if (oss8.str() != "1000000000000000000000000001000000 "
    ...
    //-8888888888884444444467967909888283332222222222232111102223777677777
  {
    throw std::runtime_error("Operator - failed. " + oss8.str());
  }
...
void testRational() {
  ...
  //Rational g = BigInteger(78) / BigInteger(200);
  //throw std::runtime_error("Test 7 failed. " + g.asDecimal(2));
  std::string fre = (s/t).asDecimal(10);
  if ((s/t).asDecimal(10) != "5337140829307966068.3989202202")
    //5337140829307966068.4911080352
